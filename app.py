...
def load_data(db_path: str):
    ...
    if not xhs.empty:
        ...
        # xhs_posts.brand_mentioned stores Olens as "OLENS" (all caps) — every other
        # brand-matching column in the app (products.brand, lihkg mentioned_brands,
        # selected_brands) uses "Olens". Left uncorrected, every `.isin(selected_brands)`
        # filter elsewhere in this file silently drops all Olens XHS posts (279 rows).
        xhs["brand_mentioned"] = xhs["brand_mentioned"].replace({"OLENS": "Olens"})
...
TRIANGULATION_FILES = {
    ...
    "Distribution Gap": ("distribution_gap.md", "distribution_gap.csv"),
...
# Praise gets its own taxonomy rather than reusing the complaint labels above —
# "product mismatch/counterfeit concern" or "quality/defect (other)" read as
# complaints even applied to a 5-star review. See PRAISE_CATEGORIES in
# triangulation/negative_tail_analysis.py, which this mirrors.
PRAISE_TAIL_CATEGORY_ORDER = [
    ...
    for label, (md_name, csv_name) in TRIANGULATION_FILES.items():
        ...
        if os.path.exists(md_path):
            try:
                ...
                # Strip the internal "Prompt A/B/C/D" research-prompt heading —
                # the expander title already shows the client-facing label.
                md_text = re.sub(r"^#\s*Prompt\s+[A-Z]\s*[—-]\s*.*\n+", "", md_text, count=1)
            except Exception:
                md_text = ""
        csv_df = pd.DataFrame()
...
@st.cache_data(show_spinner=False)
def load_monthly_trends(tri_dir: str, dir_mtime: float) -> pd.DataFrame:
    """Full month x brand grid — see triangulation/monthly_trends.py. Empty if not yet generated."""
    path = os.path.join(tri_dir, "monthly_trends.csv")
    if not os.path.exists(path):
        return pd.DataFrame()
    try:
        return pd.read_csv(path)
...
@st.cache_data(show_spinner=False)
def load_monthly_trends_events(tri_dir: str, dir_mtime: float) -> pd.DataFrame:
    """Flagged month-over-month anomalies — see triangulation/monthly_trends.py. Empty if not yet generated."""
    path = os.path.join(tri_dir, "monthly_trends_events.csv")
    if not os.path.exists(path):
        return pd.DataFrame()
    try:
        return pd.read_csv(path)
...
monthly_trends_data = (
    load_monthly_trends(triangulation_dir, tri_mtime) if os.path.isdir(triangulation_dir) else pd.DataFrame()
)
monthly_trends_events = (
    load_monthly_trends_events(triangulation_dir, tri_mtime) if os.path.isdir(triangulation_dir) else pd.DataFrame()
)
...
_SUBBRAND_PALETTE = ["#1d4ed8", "#0ea5e9", "#7c3aed", "#f59e0b", "#059669", "#db2777"]
...
with tab_reviews_sentiment:
    ...
    with sub_sentiment_pane:
        ...
        if reviews_f.empty:
            ...
            if sv.empty:
                ...
                view = st.radio(
                    "View",
                    ["📊  All Brands Overview", "🔍  Brand Deep-Dive", "🏷️  Sub-brands"],
                    horizontal=True,
                    label_visibility="collapsed",
                )
                ...
                if view == "📊  All Brands Overview":
                    ...
                    if not _sub_brand_options:
                        ...
                        if acv_sv.empty:
                            ...
                            sub_tabs = st.tabs(_subs_for_brand)
    ...
    with sub_new_wearer_pane:
        st.subheader("New Wearers — First-Time Buyers")
        st.caption(_site_caption(reviews_f, "review_date", "reviews"))
        st.caption(
        ...
        if nw_f.empty:
            ...
            if new_wearers.empty:
                ...
                st.subheader("Monthly first-time-buyer sentiment trend")
                ...
                st.subheader("First-time-buyer reviews")
...
with tab_social_signals:
    sub_xhs_pane, sub_lihkg_pane = st.tabs(["Customer Feedback (XHS)", "Customer Signals (LIHKG)"])
    with sub_xhs_pane:
        if xhs.empty:
...
# ---- Catalog Explorer ----------------------------------------------------------
with tab_catalog:
    sub_stores_pane, sub_explorer_pane = st.tabs(["Store Ranking", "Product Explorer"])
    with sub_stores_pane:
        ...
        if rated.empty:
            ...
            st.divider()
            st.subheader("Stores common across brands")
            st.caption(
...
with tab_research_triangulation:
    ...
    for label, content in (
        ...
        with st.container():
            ...
            if label == "Channel Coverage" and not content["csv"].empty:
                cc = content["csv"].copy()
                cc["scraped_label"] = cc["scraped"].map({"Y": "Scraped", "N": "Not scraped"})
                fig_cc = px.bar(
                    cc.sort_values("product_count", ascending=True),
                    x="product_count", y="taxonomy_category", color="scraped_label",
                    orientation="h",
                    color_discrete_map={"Scraped": "#16a34a", "Not scraped": "#dc2626"},
                    labels={
            ...
            if label == "Distribution Gap" and not content["csv"].empty:
                dg = content["csv"]
                fig_dg = px.bar(
                    dg.sort_values("coverage_pct", ascending=False),
                    x="brand", y="coverage_pct", color="brand",
                    color_discrete_map=BRAND_COLORS,
                    labels={"coverage_pct": "Coverage %", "brand": "Brand"},
                    title="Distribution coverage — % of researched retailers actually scraped",
                )
                fig_dg.update_layout(showlegend=False, yaxis_range=[0, 100], height=420)
                st.plotly_chart(fig_dg, width="stretch")
                st.caption(
                    "Coverage % = scraped retailers / researched (Present or Exclusive) retailers "
                    "per Research/distribution.csv, matched via a 3-entry alias map (HKTVmall, "
                    "393lens.com, CL-Mall.com) — every other researched retailer has no scraped "
            ...
            if label == "External Validation" and not content["csv"].empty:
                ...
                ev_display = ev[["brand", "scraped_lean", "reputation_lean", "agreement", "pooled_net_pct", "notes"]].copy()
                ev_display["agreement"] = ev_display["agreement"].map(lambda a: _agreement_display.get(a, a))
                ev_display = ev_display.rename(columns={
                    "brand": "Brand", "scraped_lean": "Scraped lean", "reputation_lean": "Reputation.csv lean",
                    "agreement": "Agreement", "pooled_net_pct": "Scraped net %", "notes": "Notes",
                })
                st.dataframe(ev_display, width="stretch", hide_index=True)
                st.caption(
                    "Pools sentiment across reviews + XHS posts + XHS comments + LIHKG posts (no "
                    "new LLM classification — all four already carry a sentiment label) and "
                    "compares the resulting lean against Research/reputation.csv's lean per brand. "
                    "Evidence for any Diverging brand is below."
                )
            ...
            if label == "Share of Voice" and not content["csv"].empty:
                ...
                fig.update_layout(
                    ...
                    height=520, showlegend=True, legend_title_text="Brand",
                )
                st.plotly_chart(fig, width="stretch")
                st.caption(
                ...
                if "net_sentiment_pct" in sov.columns:
                    ...
                    if "xhs_small_base_flag" in sov.columns:
                        ...
                        # Sorted by pct_products descending so the callout lands on the most
                        # isolated bubble available in each group, not whichever happens to be
                        # first in the CSV — the low-pct_products brands sit in a tight cluster
                        # and a callout there used to land right on top of another bubble's label.
                        low_conf_example = sov[sov["brand"].isin(low_conf_brands)].sort_values(
                            "pct_products", ascending=False
                        )
                        high_conf_example = sov[~sov["brand"].isin(low_conf_brands)].sort_values(
                            "pct_products", ascending=False
                        )
            ...
            if label == "-ve and +ve Tail" and not negative_tail_reviews.empty:
                ...
                if is_positive:
                    with st.expander("What's in each category?"):
                        st.markdown(
                            "Praise uses its own category wording rather than reusing the complaint "
                            "labels — a category like \"product mismatch/counterfeit concern\" reads as "
                            "a complaint even when it's really a 5-star review confirming the product is "
                            "genuine, so praise is classified separately:\n"
                            "- **authenticity / genuine product** — confirms it's the real thing, not a "
                            "counterfeit.\n"
                            "- **comfort** — comfortable to wear, no dryness/irritation.\n"
                            "- **shipping/fulfillment**, **customer service**, **packaging** — same "
                            "topics as the complaint view, praised instead.\n"
                            "- **value for money** — good price/value called out specifically.\n"
                            "- **quality/reliability (other)** — a real positive (fit, prescription, "
                            "general quality) that isn't one of the named topics above.\n"
                            "- **general satisfaction / no specific theme** — happy overall but doesn't "
                            "call out a specific thing they liked.\n"
                            "- **no praise / negative review** — a review with nothing positive to "
                            "report. Shows up as you lower the rating cutoff below 5 stars.\n"
                ...
                if is_positive:
                    # Praise is classified on its own taxonomy (praise_category), independent
                    # of the complaint "category" column — swap it in here so the rest of this
                    # block (which all reads "category") works unchanged for either direction.
                    neg_f = neg_f.copy()
                    neg_f["category"] = neg_f["praise_category"]
                ...
                if breakdown_live.empty:
                    ...
                    _category_order = PRAISE_TAIL_CATEGORY_ORDER if is_positive else NEGATIVE_TAIL_CATEGORY_ORDER
                    present_categories = [c for c in _category_order if c in breakdown_live["category"].unique()]
...
# ---- Trends & Demand -------------------------------------------------------------
with tab_trends_demand:
    sub_demand_pane, sub_monthly_pane, sub_anomaly_pane = st.tabs(
        ["Demand Signals", "Monthly Trends", "Anomaly Explorer"]
    )
    with sub_demand_pane:
        ...
        if not demand_signals.classify_trend_reliability(demand_brand):
            ...
            if monthly_search.empty:
                ...
                with st.popover("📅 Date filters", width='stretch'):
                    dfcol1, dfcol2, dfcol3 = st.columns(3)
                    with dfcol1:
                        dm_sel_years = st.multiselect("Year", _dm_years, default=_dm_years, key="dm_year")
                    with dfcol2:
                        dm_sel_quarters = st.multiselect(
                            "Quarter", [1, 2, 3, 4], default=[1, 2, 3, 4], key="dm_quarter",
                            format_func=lambda q: f"Q{q}",
                        )
                    with dfcol3:
                        dm_sel_months = st.multiselect(
                            "Month", list(range(1, 13)), default=list(range(1, 13)), key="dm_month",
                            format_func=lambda m: calendar.month_abbr[m],
                        )

                def _dm_filter(df):
                    d = pd.to_datetime(df["month"], format="%Y-%m")
                    return df[
                        d.dt.year.isin(dm_sel_years)
                        & d.dt.quarter.isin(dm_sel_quarters)
                        & d.dt.month.isin(dm_sel_months)
                    ]

                if not (dm_sel_years and dm_sel_quarters and dm_sel_months):
    ...
    with sub_monthly_pane:
        st.subheader("Monthly Trends — review and social activity over time")
        st.caption(
            "Full available date range shown (not restricted to any fixed window) — early months are "
            "sparse and will read as thin, which is honest about data coverage rather than hiding it. "
            "Regenerate via `python triangulation/monthly_trends.py` — see triangulation/monthly_trends.py."
        )

        if monthly_trends_data.empty:
            st.info(
                "No monthly trends output found yet. Run `python triangulation/monthly_trends.py` "
                "from the project root, or point the sidebar 'Triangulation analysis folder' at "
                "existing output."
            )
        else:
            mt = monthly_trends_data[monthly_trends_data["brand"].isin(selected_brands)].copy()
            mt_events = (
                monthly_trends_events[monthly_trends_events["brand"].isin(selected_brands)]
                if not monthly_trends_events.empty else monthly_trends_events
            )

            _mt_years = sorted(pd.to_datetime(mt["month"], format="%Y-%m").dt.year.unique().tolist())
            with st.popover("📅 Date filters", width='stretch'):
                mtcol1, mtcol2, mtcol3 = st.columns(3)
                with mtcol1:
                    mt_sel_years = st.multiselect("Year", _mt_years, default=_mt_years, key="mt_year")
                with mtcol2:
                    mt_sel_quarters = st.multiselect(
                        "Quarter", [1, 2, 3, 4], default=[1, 2, 3, 4], key="mt_quarter",
                        format_func=lambda q: f"Q{q}",
                    )
                with mtcol3:
                    mt_sel_months = st.multiselect(
                        "Month", list(range(1, 13)), default=list(range(1, 13)), key="mt_month",
                        format_func=lambda m: calendar.month_abbr[m],
                    )
            ...
            def _mt_filter(df):
                d = pd.to_datetime(df["month"], format="%Y-%m")
                return df[
                    d.dt.year.isin(mt_sel_years)
                    & d.dt.quarter.isin(mt_sel_quarters)
                    & d.dt.month.isin(mt_sel_months)
                ]

            if not (mt_sel_years and mt_sel_quarters and mt_sel_months):
                mt = mt.iloc[0:0]
                mt_events = mt_events.iloc[0:0]
            else:
                mt = _mt_filter(mt)
                if not mt_events.empty:
                    mt_events = _mt_filter(mt_events)
            ...
            if mt.empty:
                ...
                st.caption(
                    f"{int(mt['review_count'].sum()):,} reviews across "
                    f"{mt['brand'].nunique()} brand(s) and {mt['month'].nunique()} month(s) "
                    "in the current filter."
                )
                fig_rv = px.line(
                    mt, x="month", y="review_count", color="brand", markers=True,
                    color_discrete_map=BRAND_COLORS, labels={"review_count": "Reviews", "month": ""},
                )
                if not mt_events.empty:
                    rv_events = mt_events[mt_events["metric"] == "review_count"]
                    if not rv_events.empty:
                        merged_rv = rv_events.merge(mt[["month", "brand", "review_count"]], on=["month", "brand"], how="left")
                        fig_rv.add_scatter(
                            x=merged_rv["month"], y=merged_rv["review_count"], mode="markers",
                            marker=dict(symbol="triangle-up", size=12, color="black"),
                            name="Flagged anomaly", showlegend=True,
                        )
                fig_rv.update_layout(height=380)
                st.plotly_chart(fig_rv, width="stretch")

                st.markdown("**Average rating**")
                st.caption(
                    f"Averaged from the same {int(mt['review_count'].sum()):,} reviews "
                    "used in the Review volume chart above."
                )
                fig_ar = px.line(
                    mt, x="month", y="avg_rating", color="brand", markers=True,
                    color_discrete_map=BRAND_COLORS, labels={"avg_rating": "Avg. rating", "month": ""},
                )
                fig_ar.update_layout(height=380, yaxis_range=[1, 5])
                st.plotly_chart(fig_ar, width="stretch")

                st.markdown("**XHS volume + sentiment**")
                st.caption(
                    f"{int(mt['xhs_post_count'].sum()):,} XHS posts across "
                    f"{mt['brand'].nunique()} brand(s) and {mt['month'].nunique()} month(s) "
                    "in the current filter."
                )
                fig_xhs = make_subplots(rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.08,
                                         subplot_titles=("XHS post count", "Avg. XHS sentiment (-1 to 1)"))
                for brand in sorted(mt["brand"].unique()):
                    b = mt[mt["brand"] == brand]
                    color = BRAND_COLORS.get(brand)
                    fig_xhs.add_scatter(x=b["month"], y=b["xhs_post_count"], mode="lines+markers",
                                         name=brand, legendgroup=brand, line=dict(color=color), row=1, col=1)
                    fig_xhs.add_scatter(x=b["month"], y=b["avg_xhs_sentiment"], mode="lines+markers",
                                         name=brand, legendgroup=brand, showlegend=False,
                                         line=dict(color=color), row=2, col=1)
                fig_xhs.add_hline(y=0, line_dash="dash", line_color="#94a3b8", row=2, col=1)
                if not mt_events.empty:
                    sent_events = mt_events[mt_events["metric"] == "avg_xhs_sentiment"]
                    if not sent_events.empty:
                        merged_sent = sent_events.merge(
                            mt[["month", "brand", "avg_xhs_sentiment"]], on=["month", "brand"], how="left"
                        )
                        fig_xhs.add_scatter(
                            x=merged_sent["month"], y=merged_sent["avg_xhs_sentiment"], mode="markers",
                            marker=dict(symbol="triangle-up", size=12, color="black"),
                            name="Flagged anomaly", row=2, col=1,
                        )
                ...
                st.caption(
                    "Directional flag only — a 3-month trailing window is a small, noisy sample, not a "
                    "statistical test. These are candidates worth a closer look, not explained causes."
                )
                if mt_events.empty:
                    st.info("No anomalies flagged for the selected brands.")
                else:
                    st.dataframe(
                        mt_events.rename(columns={
                            "brand": "Brand", "month": "Month", "metric": "Metric", "value": "Value",
                            "trailing_mean": "Trailing 3-mo avg", "trailing_std": "Trailing 3-mo std",
                        }),
                        width="stretch", hide_index=True,
                    )
                ...
                with col_dl1:
                    st.download_button(
                        "Download monthly_trends.csv (full grid)",
                        monthly_trends_data.to_csv(index=False).encode("utf-8-sig"),
                        file_name="monthly_trends.csv", mime="text/csv", key="dl_monthly_trends",
                    )
                with col_dl2:
                    st.download_button(
                        "Download monthly_trends_events.csv (flagged anomalies)",
                        monthly_trends_events.to_csv(index=False).encode("utf-8-sig"),
                        file_name="monthly_trends_events.csv", mime="text/csv", key="dl_monthly_trends_events",
                    )

    with sub_anomaly_pane:
        st.subheader("Anomaly Explorer")
        st.markdown(
            "A month is **flagged** for a brand+metric when its value differs from that brand's "
            "own trailing 3-month average by more than 1 trailing standard deviation — a *directional* "
            "signal, not a statistical significance test. A brand-month is only eligible to be flagged "
            "once it has 3 full prior months of data and some real activity nearby (pure zero-padding "
            "before a brand's first real month is never flagged). With only a 3-month baseline, this "
            "threshold is loose by design — it will flag often, especially for series with a handful of "
            "reviews or posts per month. Treat flags as candidates worth a closer read, not confirmed events."
        )
        ...
        if monthly_trends_data.empty:
            ...
            _ae_grid = monthly_trends_data[monthly_trends_data["brand"].isin(selected_brands)].copy()
            _ae_events = (
                monthly_trends_events[monthly_trends_events["brand"].isin(selected_brands)].copy()
                if not monthly_trends_events.empty
                else monthly_trends_events.copy()
            )

            @st.cache_data(show_spinner=False)
            def _anomaly_eligibility_counts(grid: pd.DataFrame, window: int = 3) -> pd.DataFrame:
                """Re-derives how many brand-months were even *eligible* to be flagged (same
                rule as triangulation/monthly_trends.py's flag_anomalies), so the flag count
                can be read as a rate rather than a bare number with no denominator."""
                metrics = {"review_count": "review_count", "avg_xhs_sentiment": "xhs_post_count"}
                rows = []
                for brand, bg in grid.groupby("brand"):
                    bg = bg.sort_values("month").reset_index(drop=True)
                    for metric, activity_col in metrics.items():
                        values = bg[metric]
                        activity = bg[activity_col]
                        eligible = 0
                        for i in range(window, len(bg)):
                            if activity.iloc[i - window: i + 1].sum() == 0:
                                continue
                            trailing = values.iloc[i - window: i].dropna()
                            if len(trailing) < window or pd.isna(values.iloc[i]):
                                continue
                            eligible += 1
                        rows.append({"brand": brand, "metric": metric, "eligible_months": eligible})
                return pd.DataFrame(rows)

            _elig = _anomaly_eligibility_counts(_ae_grid)
            if not _ae_events.empty:
                _flag_counts = _ae_events.groupby(["brand", "metric"]).size().reset_index(name="flagged_months")
                _rate = _elig.merge(_flag_counts, on=["brand", "metric"], how="left")
            else:
                _rate = _elig.copy()
                _rate["flagged_months"] = 0
            _rate["flagged_months"] = _rate["flagged_months"].fillna(0).astype(int)
            _rate["flag_rate_pct"] = (_rate["flagged_months"] / _rate["eligible_months"].replace(0, pd.NA) * 100).round(0)

            m1, m2, m3 = st.columns(3)
            m1.metric("Flagged events (selected brands)", f"{len(_ae_events):,}")
            m2.metric("Eligible brand-months", f"{int(_rate['eligible_months'].sum()):,}")
            _overall_rate = (
                _rate["flagged_months"].sum() / _rate["eligible_months"].sum() * 100
                if _rate["eligible_months"].sum() else 0
            )
            m3.metric("Overall flag rate", f"{_overall_rate:.0f}%")

            st.markdown("**Flag rate by brand & metric**")
            st.caption(
                "How often an eligible month actually gets flagged — a high rate means the 1-std "
                "threshold is triggering routinely for that series (typically low, choppy volume), "
                "not that something unusual is really happening every time."
            )
            st.dataframe(
                _rate.rename(columns={
                    "brand": "Brand", "metric": "Metric",
                    "eligible_months": "Eligible months", "flagged_months": "Flagged months",
            ...
            st.markdown("---")
            st.markdown("**Drill into a specific flagged event**")

            if _ae_events.empty:
                st.info("No anomalies flagged for the selected brands.")
            else:
                _ae_events_sorted = _ae_events.sort_values(["month", "brand", "metric"], ascending=[False, True, True])
                _ae_labels = [
                    f"{r.brand} · {r.month} · {r.metric} (value={r.value}, trailing avg={r.trailing_mean})"
                    for r in _ae_events_sorted.itertuples()
                ]
                _ae_choice = st.selectbox("Flagged event", _ae_labels, key="ae_event_choice")
                _ae_row = _ae_events_sorted.iloc[_ae_labels.index(_ae_choice)]

                _direction = "Spike (above trailing average)" if _ae_row["value"] > _ae_row["trailing_mean"] else "Drop (below trailing average)"
                dc1, dc2, dc3, dc4 = st.columns(4)
                dc1.metric("Brand", _ae_row["brand"])
                dc2.metric("Month", _ae_row["month"])
                dc3.metric("Value", _ae_row["value"])
                dc4.metric("Trailing 3-mo avg (±std)", f"{_ae_row['trailing_mean']} (±{_ae_row['trailing_std']})")
                st.caption(_direction)

                _ae_period = pd.Period(_ae_row["month"], freq="M")

                if _ae_row["metric"] == "review_count":
                    st.markdown(f"**Reviews behind this flag — {_ae_row['brand']}, {_ae_row['month']}**")
                    _drill_reviews = reviews_f[
                        (reviews_f["brand"] == _ae_row["brand"])
                        & (reviews_f["review_date"].dt.to_period("M") == _ae_period)
                    ].sort_values("review_date")
                    if _drill_reviews.empty:
                        st.info("No matching reviews found in the currently loaded data for this brand-month.")
                    else:
                        st.dataframe(
                            _drill_reviews[["store_name", "review_date", "rating", "review_text_en"]].rename(columns={
                                "store_name": "Store", "review_date": "Date",
                                "rating": "Rating", "review_text_en": "Review (EN)",
                            }),
                    ...
                    st.markdown(f"**XHS posts behind this flag — {_ae_row['brand']}, {_ae_row['month']}**")
                    _drill_xhs = xhs[
                        (xhs["brand_mentioned"] == _ae_row["brand"])
                        & (xhs["publish_date"].dt.to_period("M") == _ae_period)
                    ].sort_values("publish_date")
                    if _drill_xhs.empty:
                        st.info("No matching XHS posts found in the currently loaded data for this brand-month.")
                    else:
                        st.dataframe(
                            _drill_xhs[["publish_date", "title", "content_en", "sentiment", "likes"]].rename(columns={
                                "publish_date": "Date", "title": "Title", "content_en": "Content (EN)",
