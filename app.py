...
_ACUVUE_SUBS = ["Moist", "Oneday", "Define", "Max"]
_ACUVUE_SUB_COLORS = {
    "Acuvue – Moist":  "#1d4ed8",
...
def _acuvue_subbrand(name):
    """"Max" (Acuvue Oasys Max 1-Day) is checked before the generic 1-day
    bucket so it doesn't get folded into "Oneday" alongside plain Oasys
    1-Day / HydraLuxe products — it's J&J's newer, separately-marketed line."""
    if pd.isna(name):
        return "Other"
    n = name.lower()
    if "max" in n:
...
with tab_reviews_sentiment:
    sub_review_pane, sub_sentiment_pane = st.tabs(["Review Intelligence", "Sentiment Intelligence"])
            ...
            if sv.empty:
                ...
                if view == "📊  All Brands Overview":
                    ...
                    if acv_sv.empty:
                        ...
                        grid_sub = st.columns(len(_ACUVUE_SUBS))
...
with tab_research_triangulation:
    sub_research_pane, sub_triangulation_pane = st.tabs(["Research Findings", "Triangulation Analysis"])
    with sub_research_pane:
    ...
    with sub_triangulation_pane:
        ...
        if not any(v["md"] for v in triangulation_data.values()):
            ...
            pillar_tabs = st.tabs(list(triangulation_data.keys()))
...
with tab_trends_demand:
    sub_demand_pane, sub_monthly_pane = st.tabs(["Demand Signals", "Monthly Trends"])
